 ...
 * If an id attribute was not passed an automatic id will be generated as the component relies in part on the "for"
 * attribute of the label element it generates
...
class CheckBox extends ClassEnforcedContainerElement{
	...
	public function __construct($id, $label='', $value='', $checked=false, $disabled=false, $class='', $title='', $style='', $indentContents=true){
		...
		$this->_input = new InputElement('checkbox', $id, $attr, $id);
		...
		//Creating element
		parent::__construct(array('checkbox'), 'div', array($this->_label), '', $class, $title, $style, $indentContents);
