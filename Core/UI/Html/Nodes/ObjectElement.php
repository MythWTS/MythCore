...
/**
 * Represents a "object" html element in the documents object model
 * Constructor($content, $id='', $class='', $title='', $style='', $indentContent=true)
 */
class ObjectElement extends GenericContainerElement{
	/** Constructor($content, $id='', $class='', $title='', $style='', $indentContent=true) */
	public function __construct($data='', $type='', $id='', $class='', $title='', $style='', $indentContent=true){
		parent::__construct(Tags::$OBJECT, null, null, $id, $class, $title, $style, $indentContent);
		if(!U::NA($data)){$this->_attributes['data'] = U::ES($data);}
		if(!U::NA($type)){$this->_attributes['type'] = U::ES($type);}
	}
	###########################################################################
	public function AddNode(ParamElement $param){
		$this->_contents[] = $param;
		return $this;
	}
	public function AddParameter($name, $value){
		$this->_contents[] = new ParamElement($name, $value);
		return $this;
	}
	###########################################################################
