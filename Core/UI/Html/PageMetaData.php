...
/**
 * A data structure to hold the meta data of an html page including:
 * -Regular meta data such as link and meta tag data
 * -Facebook meta data
 * -Twitter meta data
 * -Schema.org meta data
 */
class PageMetaData extends Object{
	############################################################################
	# Protected Fields
	############################################################################
	...
	/**
	 * Constructor(
	 * 		$title='New Page', $desc='New Page', $keywords='', DocTypes $docType=null, $charset='UTF-8',
	 * 		FBMetaData $fbData = null, TwitterCard $twitterCard = null, SOMetaData $soData = null
	 * 		)
	 */
	public function __construct(
			$title='', $desc='', $keywords='', DocTypes $docType=null, $charset='UTF-8',
			FBMetaData $fbData = null, TwitterCard $twitterCard = null, SOMetaData $soData = null){
		$this->_docType = $docType?:DocTypes::$Html5;
		$this->_charset = $charset;
		$this->_fbData = $fbData?: new FBMetaData();
		$this->_twitterData = $twitterCard?: new TwitterCard();
		$this->_soData = $soData?: new SOMetaData();
		$this->_title = $title;
	...
	############################################################################
	public function FBData(FBMetaData $value=null){
		if($value === null){return $this->_fbData;}
		else{$this->_fbData = $value;}
	}
	public function TwitterCard(TwitterCard $value=null){
		if($value === null){return $this->_twitterData;}
		else{$this->_twitterData = $value;}
	}
	public function SchemaOrgData(SOMetaData $value=null){
	...
	public function GenerateHeadElement($id='', $indentContents=true){
		$res = new HeadElement($id, $indentContents);
	...
	public function GenerateDocTypeNode(){
		return new DocTypeNode($this->_docType);
	}
};
?>