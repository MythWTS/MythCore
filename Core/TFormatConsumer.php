 ...
 * <b>THIS TRAIT CREATES INSTANCE METHODS. IF YOU WANT TO ADD THE FUNCTIONALITY TO A STATIC/UTILITY CLASS, USE TStaticFormatConsumer</b><br/>
 ...
 * </li>
 * </ul>
 * @see \Core\TStaticFormatConsumer
 * @see \Core\Strings::Extract()
 */
trait TFormatConsumer{
	###########################################################################
	# Static Fields
	###########################################################################
	/**
				...
				elseif($res !== 0){
					...
					foreach ($matches as $match){
						...
						if($config->GlobalFormattersCount > 0){
							$res = $this->tryFormatters($config->GlobalFormatters(), $formatOptions, $objectToFormat);
							if($res !== null){
						...
						if(!$found){
							...
							if($config->HasFormattersForType($typeName)){
								$res = $this->tryFormatters($config->GetFormatterForType($typeName), $formatOptions, $objectToFormat);
								if($res !== null){
									$found = true; $replacement = $res;
								}
							}
						...
						if(!$found){
							$replacement = U::ES($objectToFormat);
						}
	 ...
	 * @return NULL|string
	 */
	private function tryFormatters(array $formatters, $formatOptions, $objectToFormat){
		$return = null;
